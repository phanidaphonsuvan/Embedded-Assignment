...
uint8_t buf[32];
float accbuffer[SIZEBUFFER * NB_AXES] = {0};
float class_buffer_output[CLASS_NUMBER]; // Buffer of class probabilities
	  ...
	  for (uint16_t i = 0; i < SIZEBUFFER; i++) {
		 ...
		 if (ret != HAL_OK) {
			 ...
			 if (ret != HAL_OK) {
				 ...
				 accbuffer[NB_AXES * i] = ((buf[1] << 8) | buf[0]);
				 accbuffer[(NB_AXES * i) + 1] = ((buf[3] << 8) | buf[2]);
				 accbuffer[(NB_AXES * i) + 2] = ((buf[5] << 8) | buf[4]);
			 }
		 }
	 }

	 neai_classification(accbuffer, class_buffer_output, &newClass);
